    ...
    <div className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-transform duration-300 ease-in-out transform hover:-translate-y-1 flex flex-col overflow-hidden border border-gray-100">
      ...
      <div className="relative aspect-square overflow-hidden bg-gray-200">
        <img
          src={
            image_url ||
            `https://placehold.co/400x400/CCCCCC/333333?text=${name
              .slice(0, 15)
              .replace(/\s/g, "+")}`
          }
          alt={name}
          ...
          onError={(e) => {
            ...
            ).src = `https://placehold.co/400x400/CCCCCC/333333?text=Image+Not+Found`;
          }}
        />
        {/* Stock Status Badge */}
        <span
          className={`absolute top-3 right-3 text-xs font-semibold px-3 py-1 rounded-full ${getStockBadge()}`}
        >
          {isOutOfStock
            ? "Out of Stock"
            : stock_quantity < 50
      ...
      <div className="p-4 flex flex-col flex-grow">
        ...
        <div className="flex items-center mb-3">
          ...
          <span className="text-sm font-bold text-gray-700">
            {numericRating > 0 ? formattedRating : "N/A"}
          </span>
